...
static int find_free(int num_chunks) {
	// my code: next-fit implmentation
	/*
		Next fit is a modification of frst-fit where the algorithm picks up 
		its seach frim where the last first-fit case left off
	*/
	static int i = 0;	// preserve i between calls
	int j, k;

	for(k=0; k<max_chunk; k++) {
	...
	/*
	// weaver's code: first-fit implementation
	int i,j;
